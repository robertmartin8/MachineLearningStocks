...
def build_dataset_iteratively(idx_start, idx_end, date_start=START_DATE, date_end=END_DATE):
    """
    This is an iterative solution to building the stock dataset, which may be necessary if the
...
def build_stock_dataset(start=START_DATE, end=END_DATE):
    """
    Creates the dataset that will hold all stock prices.
    ...
    # Remove any columns that hold no data
